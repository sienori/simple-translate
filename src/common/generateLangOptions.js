...
// OpenAI supports all languages, so we'll use the same list as Google
const langListOpenAI = langListGoogle;

export default (translationApi) => {
  let langList;
  if (translationApi === "google") {
    langList = langListGoogle;
  } else if (translationApi === "deepl") {
    langList = langListDeepl;
  } else if (translationApi === "openai") {
    langList = langListOpenAI;
  }
